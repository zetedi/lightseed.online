...
### Step 2: Configure Email (Google Workspace)
To use your Google Workspace email (`you@lifeseed.online`) with the "Trigger Email" extension:

1. **Enable 2-Step Verification** in your Google Account settings.
2. Go to **Security** > **App Passwords** (search for it if not visible).
3. Create a new App Password named "Firebase".
4. Copy the 16-character password generated by Google.
5. In the Firebase Extension configuration, set the **SMTP Connection URI** to:
   ```
   smtps://yourname@lifeseed.online:YOUR_16_CHAR_PASSWORD@smtp.gmail.com:465
   ```
