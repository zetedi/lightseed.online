...
export const Modal = ({ children, onClose, title, backgroundImage }: ModalProps) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
        ...
        >
            <div className={`shrink-0 px-4 py-3 border-b flex justify-between items-center ${backgroundImage ? 'bg-black/40 border-white/20 backdrop-blur-md' : 'bg-slate-50 border-slate-100'}`}>
                <h3 className={`font-semibold ${backgroundImage ? 'text-white drop-shadow-md' : 'text-slate-800'}`}>{title}</h3>
            ...
            <div className={`flex-1 overflow-y-auto p-4 ${backgroundImage ? 'bg-black/50 backdrop-blur-sm' : ''}`}>
                {children}
            </div>
        </div>
    </div>
