...
const callGemini = async (prompt: string, model: string = 'gemini-2.5-flash', config?: any): Promise<{text?: string, image?: string}> => {
    try {
...
export const generateImage = async (prompt: string): Promise<string | null> => {
    ...
    try {
        ...
        const res = await callGemini(prompt, 'gemini-2.5-flash-image', { imageConfig: { aspectRatio: "1:1" } });
