// THE VOICE OF A LETTER (ring 2026-09-14). A system mail is sent FROM a place: the community
// rooted at the door the hand stood at, or the node itself. The place's name already signs the
// sender line (charter mailFromOf); this law dresses the letter's BODY in the place's own
// voice — its name at the head, its palette on the button and the paper, a greeting before
// the words and a signature after them, its own footer line — all DATA a keeper writes on
// the Appearance tab, never code per place. Nothing here composes HTML: the shape is this
// law's, the markup the server's. MIRROR: functions/src/mailVoice.ts (tests hold them equal).
//
// Plain contract — guaranteed: every line is stripped of control characters, whitespace one
// breath wide, at most MAIL_LINE_MAX long; a colour is a six-digit hex or the shell's own; the
// ink and paper are taken together only when they read (contrast >= 3), else both stay the
// shell's; a dark-authored theme lends nothing to paper (mail is read on white). Not
// guaranteed: that the words are kind, or in the reader's tongue.
