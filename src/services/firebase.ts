...
// Basic fetchers (Used by Infinite Query Hooks directly now mostly)
export const fetchLifetrees = async (): Promise<Lifetree[]> => {
  const q = query(lifetreesCollection, orderBy('createdAt', 'desc'), limit(20));
...
// ... Pulses, Matches, Visions helpers remain similar but optimized ...

export const fetchPulses = async (): Promise<Pulse[]> => {
  const q = query(pulsesCollection, orderBy('createdAt', 'desc'), limit(20));
  const querySnapshot = await getDocs(q);
...
// ... Export other helpers like getMyPulses, createVision, match logic ...
