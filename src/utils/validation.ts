...
// A tree's validation is not a permanent stamp; it stays lit only while the tree is cared for
// (a real growth pulse or an explicit confirm) within the window. Left uncared for, it DIMS — the
// stamp remains, but it no longer counts until re-cared for. You keep standing by caring.
...
// When the tree was last cared for — last explicit care / growth, falling back to update/birth.
export const lastCaredMillis = (tree?: Pick<Lifetree, 'lastCaredAt' | 'updatedAt' | 'createdAt'> | null): number =>
  toMs((tree as any)?.lastCaredAt) || toMs((tree as any)?.updatedAt) || toMs((tree as any)?.createdAt) || 0;

// Validated AND cared for within the window — the badge is lit and the standing counts.
export const isValidationLive = (tree?: Lifetree | null, now: number = Date.now()): boolean =>
  isExplicitlyValidatedTree(tree) && (now - lastCaredMillis(tree) < VALIDATION_WINDOW_MS);

// Validated, but gone quiet — needs caring to re-light.
...
export const isValidationFading = (tree?: Lifetree | null, now: number = Date.now()): boolean => {
  ...
  const remaining = VALIDATION_WINDOW_MS - (now - lastCaredMillis(tree));
...
export const daysUntilLapse = (tree?: Lifetree | null, now: number = Date.now()): number =>
  Math.max(0, Math.ceil((VALIDATION_WINDOW_MS - (now - lastCaredMillis(tree))) / (24 * 3600 * 1000)));
...
export const canValidateTree = ({
  ...
  // Only a LIVE (caredFor) validator can pass validation on — a lapsed one must care first.
