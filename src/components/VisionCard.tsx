...
export const VisionCard = ({ vision, density = 'cards' }: { vision: Vision; density?: ListDensity }) => {
    ...
    const heading = (isRoot ? (vision.body || vision.description) : vision.title) || t('vision');
    ...
    return (
        <div className={`flex h-60 flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm ${POP} group`}>
            ...
            <div className="flex flex-1 flex-col p-3">
                <p dir="auto" className="text-slate-600 text-xs font-light italic leading-relaxed line-clamp-3 dark:text-slate-300">
                    {subtext ? `"${subtext}"` : ' '}
                </p>
            </div>
        </div>
