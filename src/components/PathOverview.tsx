    ...
    <div className="relative">
      ...
      <ol className="space-y-4">
            ...
            <li key={step.key} className="relative flex gap-4 pl-0">
              <span
                className={`z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 text-xs font-bold ${
                    ...
                    : here
                      ? 'border-emerald-500 bg-white text-emerald-600 ring-4 ring-emerald-100 dark:bg-slate-900 dark:text-emerald-300'
              ...
              <div className={`min-w-0 pb-1 ${done ? 'opacity-60' : ''}`}>
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">{t(`stage_${stage}` as Key)}{here && <span className="ml-2 rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{t('path_you_are_here')}</span>}</p>
        ...
        <li className="relative flex gap-4">
          <span className={`z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 text-sm ${current === 'sovereign' ? 'border-amber-400 bg-amber-400 text-white' : 'border-amber-200 bg-white text-amber-400 dark:border-amber-900 dark:bg-slate-900'}`}>☀</span>
