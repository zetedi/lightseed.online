    ...
    return (
        <>
            <button
                type="button"
                onClick={show}
                title={t('qr_offline_title')}
                aria-label={t('qr_for_name').replace('{name}', name)}
            ...
            {open && (
                <Modal title={name} onClose={() => setOpen(false)}>
                    <div className="flex flex-col items-center gap-4 text-center">
                        {dataUrl
                            ? <img src={dataUrl} alt={t('qr_linking_to').replace('{name}', name)} className="h-56 w-56 rounded-xl border border-slate-200 shadow-sm dark:border-slate-700" />
